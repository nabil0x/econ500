...
| 4 | Show: Convexity of preference ⇔ USC(x) is convex ⇔ u(.) is quasiconcave. | 5 | MainQ Tut1, Batch 47 Q2b | —, 2022 |
...
| 7 | Show: Strong Monotonicity ⇒ Monotonicity ⇒ LNS. | 5 | MainQ Tut1, Batch 47 Q4b | —, 2022 |
...
**A:**
• Consumer has a well-defined preference relation ≥ over consumption bundles X = Rⁿ₊ and chooses the most-preferred affordable bundle.
• **Completeness:** For any x,y, either x ≥ y or y ≥ x (or both). Consumer can compare any two bundles.
• **Transitivity:** If x ≥ y and y ≥ z then x ≥ z. Prevents cycles and money-pump exploitation.
• **Reflexivity** (implied): x ≥ x for all x.
...
### Question 2
**Q:** Define indifference set. Show strong monotonicity ⇒ ICs must be downward sloping.
**A:**
• **Indifference set** IND(x) = {y in X : y ~ x} = UCS(x) ∩ LCS(x).
• **Strong monotonicity:** If x_k ≥ y_k for all k and x_l > y_l for at least one good, then x > y.
• **Proof by contradiction:** If IC were upward-sloping, pick y >> x on same IC ⇒ y > x (by strong monotonicity) but y ~ x (by IC definition) — contradiction.
...
**A:**
• **Definition 1 (convex preferences):** If x ≥ y then αx + (1-α)y ≥ y for any α in (0,1).
• **Definition 2 (convex UCS):** The upper contour set UCS(x) = {y : y ≥ x} is convex for every x.
• The two definitions are equivalent — both capture diversification preference.
• **Strong convexity:** αx + (1-α)y > z when x≠y and both ≥ z. Indifference curves strictly bowed inward.
• Weak convexity allows flat segments (perfect substitutes); strong convexity implies strictly diminishing MRS.
**→ See:** answers-consumer-theory.md for full derivation

### Question 4
**Q:** Show: Convexity of preference ⇔ UCS(x) convex ⇔ u(.) quasiconcave.
**A:**
• (1) Convex preference ⇔ convex UCS is definitional (by Def 2).
• (2) UCS convex ⇔ u quasiconcave: Quasiconcave means u(λx + (1-λ)y) ≥ min{u(x), u(y)}.
• **(⇒)** If UCS convex, take x,y in UCS(z) ⇒ λx + (1-λ)y in UCS(z) ⇒ quasiconcavity.
• **(⇐)** If u quasiconcave, for any x,y with u(x),u(y) ≥ u(z), the convex combination also has u ≥ u(z) ⇒ UCS convex.
• Chain complete: Convex preference ⇔ convex UCS ⇔ quasiconcave utility.
**→ See:** answers-consumer-theory.md for full derivation

### Question 5
**Q:** Prove: If preference satisfies monotonicity and continuity, there exists a utility function u(.) representing it.
**A:**
• **Debreu's theorem:** Monotonicity + continuity on X = Rⁿ₊ ⇒ continuous utility function exists.
• **Proof sketch:** Let **0** = (0,...,0). For bundle x, let m = max{x₁,...,xₙ}. Define M = (m,...,m) on the 45° line.
• Monotonicity: M ≥ x ≥ **0**. Define A = {t in [0,m] : (t,...,t) ≤ x}, B = {t in [0,m] : (t,...,t) ≥ x}.
• A and B partition [0,m]; by continuity both are closed ⇒ intersection non-empty ⇒ unique t(x) ~ x.
• Set u(x) = t(x). Then x ≥ y ⇔ t(x) ≥ t(y) ⇔ u(x) ≥ u(y).
**→ See:** answers-consumer-theory.md for full derivation

### Question 6
**Q:** Define utility function. Explain desirability by monotonicity and strong monotonicity.
**A:**
• **Utility function:** u: X → R such that x ≥ y ⇔ u(x) ≥ u(y). Only ordinal — any strictly increasing transform represents same preferences.
• **Monotonicity:** If x_k ≥ y_k for all k, then x ≥ y; if all components strictly larger, x > y.
• **Strong monotonicity:** If x_k ≥ y_k for all k and at least one strictly larger, then x > y.
...
### Question 7
**Q:** Show: Strong Monotonicity ⇒ Monotonicity ⇒ LNS.
**A:**
• **(SM ⇒ M):** If x_k ≥ y_k for all k and x_l > y_l for some l, SM gives x > y. If x=y, x~y. Either way x ≥ y — so monotonicity holds.
• **(M ⇒ LNS):** For any x and ε>0, construct y = (x₁+δ,...,xₙ+δ) with δ = ε/√N. Then y_k > x_k for all k ⇒ y > x by monotonicity. ||y-x|| = ε, so LNS satisfied.
...
**A:**
• **(i) Homogeneous of degree k:** u(ax) = aᵏ u(x). MRS is constant along rays from origin. Examples: Cobb-Douglas HD(α+β), linear HD(1), Leontief HD(1).
...
• Homothetic ⇒ Engel curves are straight lines through origin; income shares constant.
**→ See:** answers-consumer-theory.md for full derivation

### Question 9
**Q:** Prove quasi-concave utility U(X,Y) is identical to dMRS/dX < 0.
**A:**
• MRS = MUₓ/MUᵧ = (∂u/∂x)/(∂u/∂y). Along IC, du = 0 ⇒ dy/dx = -MRS.
• Quasiconcavity requires bordered Hessian condition: 2uₓuᵧu_xy - uₓ²u_yy - uᵧ²u_xx ≥ 0.
• Differentiate MRS along IC: dMRS/dx = [uₓ²u_yy - 2uₓuᵧu_xy + uᵧ²u_xx] / uᵧ³.
• Numerator of dMRS/dx = -(bordered Hessian numerator). So dMRS/dx < 0 ⇔ numerator < 0 ⇔ quasiconcavity.
...
**A:**
• MUₓ = 1, MUᵧ = 1/y ⇒ MRS = y.
• Along IC: du = dx + (1/y)dy = 0 ⇒ dy/dx = -y.
• dMRS/dx = dy/dx = -y < 0 for any positive y. Hence MRS diminishes ⇒ **convex ICs**.
...
**A:**
• U = x₁^α x₂^β, budget p₁x₁ + p₂x₂ = w. Lagrangian FOCs give tangency: (αx₂)/(βx₁) = p₁/p₂ ⇒ x₂ = (β/α)(p₁/p₂)x₁.
• **Walrasian demands:** x₁ = α/(α+β) · w/p₁, x₂ = β/(α+β) · w/p₂.
• Properties: HD(0), Walras' Law holds, ∂x₁/∂p₁ < 0 (usual good), ∂x₁/∂p₂ = 0 (no cross-price effect), both goods normal.
• **Constant expenditure shares:** s₁ = α/(α+β), s₂ = β/(α+β) — independent of p and w.
**→ See:** answers-consumer-theory.md for full derivation

### Question 2
**Q:** Cobb-Douglas: Get indirect utility, explain properties, verify Roy's identity.
**A:**
• **Indirect utility:** v(p,w) = [α^αβ^β/(α+β)^(α+β)] · w^(α+β) · p₁^(-α) · p₂^(-β).
• Properties: HD(0) in (p,w), ↑ in w, ↓ in p_k, quasiconvex in (p,w).
• **Roy's identity:** x_k = -(∂v/∂p_k)/(∂v/∂w). For good 1: ∂v/∂p₁ = -αv/p₁, ∂v/∂w = (α+β)v/w ⇒ -(∂v/∂p₁)/(∂v/∂w) = αw/((α+β)p₁) = x₁ ✓
**→ See:** answers-consumer-theory.md for full derivation

### Question 3
**Q:** Cobb-Douglas: Solve expenditure minimization, examine compensated demand properties.
**A:**
• **EMP:** Minimize p₁x₁ + p₂x₂ s.t. x₁^αx₂^β ≥ ū. Tangency yields same ratio: x₂ = (β/α)(p₁/p₂)x₁.
• **Hicksian demands:** h₁ = ū^(1/(α+β)) · (α/β)^(β/(α+β)) · (p₂/p₁)^(β/(α+β)), h₂ symmetric.
• **Expenditure function:** e(p,ū) = ū^(1/(α+β)) · (α+β) · (p₁/α)^(α/(α+β))(p₂/β)^(β/(α+β)).
• Properties: HD(0) in p, compensated law of demand (∂h₁/∂p₁ < 0), duality: e(p,v(p,w)) = w and v(p,e(p,ū)) = ū.
**→ See:** answers-consumer-theory.md for full derivation

### Question 4
**Q:** Examine properties of Walrasian demand x(p,w). When can UMP sufficient condition be violated?
**A:**
• **Properties:** (1) HD(0): x(ap,aw) = x(p,w). (2) Walras' Law: p·x = w (by LNS). (3) Convex set if preferences convex; single-valued if strictly convex. (4) Continuous if u continuous and strictly quasiconcave.
• **Violations of Kuhn-Tucker sufficient conditions:**
  • **(A) Non-monotone utility:** Corner solution — MRS ≠ p₁/p₂ at optimum (e.g., bliss point).
...
**Q:** Examine: (i) Homogeneity (ii) Homotheticity.
**A:**
• **(i) Homogeneity of Walrasian demand:** x(ap,aw) = x(p,w) ∀ a > 0. Budget set unchanged. No special preferences needed beyond continuity.
• **(ii) Homothetic preferences:** If x ~ y then ax ~ ay ∀ a > 0. MRS depends only on ratio, not scale.
...
• Homogeneous utility ⇒ homothetic preferences; converse not true.
**→ See:** answers-consumer-theory.md for full derivation
...
• **Expenditure function:** e(p,ū) = ū · (p_G + p_V + p_C). HD(1) in p, Shepard's lemma: ∂e/∂p_C = ū = h_C ✓.
...
• **Demands:** x = I/pₓ - 1 (if I ≥ pₓ), y = pₓ/pᵧ.
• y is income-inelastic (∂y/∂I = 0) — quasilinear good with zero income effect. x absorbs all income changes.
...
| # | Question | Marks | Source | Year |
|---|----------|-------|--------|------|
| 1 | Demand: X = αP_X^γ P_Z^δ P_W^ε I^η. Find cross-price elasticity with Z, income elasticity. Given α=-0.5, γ=-0.4, δ=0.8: Is X luxury/necessity/inferior? Is Z substitute/complement? If P_W↓5%, ΔQ_d? If P_X↓, does expenditure↑↓? | ~11 | Batch 48 Q2b | 2023 |
| 2 | Burger King demand for Whopper: Q = 10 + 1.3P_B + 0.4P_M - 3P_F + 2.2I. What's the terrible problem? | 2 | Batch 48 Q3a | 2023 |
| 3 | Corrected demand: Q = 150P_B^{-1.5}P_{r1}^{0.75}P_{r2}^{-0.25}I^{0.8}A^{0.05}. Which good is French fries? Luxury? Advertising↑50%→ΔQ? French fry price↓50%→ΔQ? MC=$1, profit-max P? McDonald's↓10%, what % must BK lower price? | 12 | Batch 48 Q3b | 2023 |

### Question 1
**Q:** Demand: X = αP_X^γ P_Z^δ P_W^ε I^η. Find cross-price elasticity with Z, income elasticity. Given α=-0.5, γ=-0.4, δ=0.8: classify X, Z, and predict changes.
**A:**
• **Cross-price elasticity with Z:** ε_X,P_Z = δ = 0.8 > 0 ⇒ Z is a **gross substitute** for X.
• **Income elasticity:** ε_X,I = η. Classification: η > 1 luxury, 0<η<1 necessity, η<0 inferior. (Note: α is a scale constant, NOT income elasticity.)
• **P_W falls 5%:** %ΔQ = ε × (-5%). If ε > 0, demand falls; if ε < 0, demand rises.
• **P_X decreases:** |γ| = 0.4 < 1 (inelastic) ⇒ P_X ↓ ⇒ expenditure ↓ (total revenue test).
...
**A:**
• Coefficient on own price P_B is **positive** (+1.3): ∂Q/∂P_B = 1.3 > 0 ⇒ demand curve slopes **upward**.
...
**A:**
• **French fries = r2** (ε = -0.25 < 0 ⇒ complement). r1 (ε = 0.75 > 0) is a substitute (e.g., chicken sandwich).
...
• **Profit-max P (MC=$1):** Lerner Index: P(1 + 1/ε) = MC ⇒ P(1 - 2/3) = 1 ⇒ P* = $3. Markup = (3-1)/3 = 2/3.
...
• **Production set:** Y = {y ∈ Rᴸ : F(y) ≤ 0} — all technologically feasible production plans (positive = outputs, negative = inputs).
• **Transformation function:** F(y) — implicit function where F(y) ≤ 0 iff y feasible.
• **Transformation frontier:** {y ∈ Rᴸ : F(y) = 0} — boundary where firm is technically efficient.
• **MRPT** = (∂F/∂y_k)/(∂F/∂y_l) = -dy_l/dy_k along F(y)=0.
**→ See:** answers-demand-production.md for full derivation

### Question 2 & 3
**Q:** Graphically explain properties of the production set.
**A:**
• Properties: (1) Non-empty, (2) Closed, (3) No free lunch (y≥0 ⇒ y=0), (4) Possibility of inaction (0∈Y), (5) Free disposal (y'≤y ⇒ y'∈Y), (6) Irreversibility (y≠0 ⇒ -y∉Y), (7) Non-increasing RTS, (8) Non-decreasing RTS, (9) CRS, (10) Additivity/free entry, (11) Convexity.
...
**A:**
• **EoS:** σ = %Δ(K/L) / %ΔMRTS = d(K/L)/d(MRTS) · MRTS/(K/L) = ∂ln(K/L)/∂ln(MRTS).
• **Linear q = aK + bL (perfect substitutes):** MRTS = b/a constant ⇒ %ΔMRTS = 0 ⇒ σ = ∞.
• **Fixed proportions (Leontief):** K/L fixed ⇒ %Δ(K/L) = 0 ⇒ σ = 0.
• Notable: Cobb-Douglas ⇒ σ = 1. CES ⇒ σ = 1/(1-ρ).
**→ See:** answers-demand-production.md for full derivation
...
|----------|-------------|
| **1. HD(1) in w** | c(λw,q) = λc(w,q). Doubling all input prices doubles cost. |
| **2. Non-decreasing in q** | q₁ > q₀ ⇒ c(w,q₁) ≥ c(w,q₀). More output cannot cost less. |
| **3. Concave in w** | Cost rises less than linearly as input prices rise — firm substitutes away from expensive inputs. |
| **4. Shephard's Lemma** | ∂c(w,q)/∂w_k = z_k(w,q). Derivative wrt input price = conditional factor demand. |
**→ See:** answers-demand-production.md for full derivation
...
• **Cost function:** c(Q) = p_P·3Q + p_V·2Q = Q(3p_P + 2p_V). If p_P = p_V = 1, c(Q) = 5Q.
• **Profit-max P:** π = P·200P^{-6} - 5·200P^{-6} = 200P^{-5} - 1000P^{-6}. FOC ⇒ P* = 6.
**→ See:** answers-demand-production.md for full derivation
...
• **Returns to scale:** Q(tK,tL) = t^{2/3}Q(K,L), sum of exponents = 2/3 < 1 ⇒ **DRS**.
• **EoS:** σ = 1 (Cobb-Douglas).
• **LR conditional demands:** L = (r/w)^{1/2} Q^{3/2}, K = (w/r)^{1/2} Q^{3/2}.
• **LR cost:** LRTC(Q) = 2√(wr) · Q^{3/2}. **LR supply:** Qˢ = P²/(9wr) (upward-sloping for DRS).
**→ See:** answers-demand-production.md for full derivation
...
**A:**
• **Profit function:** π(p) = max_{y∈Y} p·y — value function of the PMP.
• **Properties:**
  • **HD(1) in p:** π(λp) = λπ(p) for λ > 0.
  • **Convex in p:** π(αp+(1-α)p') ≤ απ(p)+(1-α)π(p') — firm benefits from extreme price vectors.
  • **Non-decreasing in output prices, non-increasing in input prices** (Hotelling's Lemma).
  • **Continuous in p** (given Y closed).
• **Hotelling's Lemma:** ∂π/∂p_k = y_k(p). **Law of Supply:** ∂y_k/∂p_k ≥ 0.
...
**A:**
• Total differentiate transformation function F(q_x,q_y,z)=0 along frontier: (∂F/∂q_x)dq_x + (∂F/∂q_y)dq_y = 0 ⇒ MRPT = -dq_y/dq_x = (∂F/∂q_x)/(∂F/∂q_y).
...
**A:**
• **SR supply per firm:** MC = q+2. min AVC = 2 at q=0. q(P) = P - 2 for P ≥ 2.
• **Industry SR supply:** Q_S = 1,000(P - 2) for P ≥ 2.
• **Equilibrium:** 1,000(P-2) = 44,000 - 1,000P ⇒ P* = 23, Q* = 21,000. Each firm q = 21, π = 140.5.
• **$20 tax:** Supply becomes P - 20 = q+2 ⇒ q = P-22. New equilibrium: P_b = 33 (buyer), P_s = 13 (seller), Q = 11,000. Tax revenue = 220,000. Per-firm profit = -19.5 (losses but continue since P_s > min AVC).
...
**A:**
• **Walras' law:** p₁Z₁ + p₂Z₂ + p₃Z₃ = 0 ⇒ Z₁ = -(p₂/p₁)Z₂ - (p₃/p₁)Z₃.
• Substitute: Z₁ = -4 + (6p₃ - p₂)/p₁.
• Set Z₂ = Z₃ = 0. Let r₂ = p₂/p₁, r₃ = p₃/p₁.
• Z₂ = 0 ⇒ r₃ + 1 = 3r₂. Z₃ = 0 ⇒ 3 + 4r₂ = 7r₃.
...
| 3 | Cobb-Douglas u = x₁^½ x₂^½. Given {p⁰,m⁰}={{4,4},300}, {p¹,m¹}={{2,4},300}. Compute EV, CV, CS. Examine relationship → nature of commodities. | 10/5 | MainQ Tut2 Q3b, Batch 47 Q6c | —, 2022 |
...
• **CS** = area under Marshallian demand between old and new prices: ∫ x₁(p₁,·) dp₁.
...
• **Intuition:** For normal goods, price fall increases real income ⇒ consumer buys more if compensated ⇒ EV > CV. The gap reflects the income effect.
**→ See:** answers-welfare.md for full derivation
...
• Expenditure function (α=½): e(p,u) = 2u√(p₁p₂).
• CV = w - e(p¹,u⁰) = 300 - 2·37.5·√(2·4) = 300 - 212.1 = **87.9**.
• EV = e(p⁰,u¹) - w = 2·53.03·√(4·4) - 300 = 424.24 - 300 = **124.24**.
• CS = ∫₄² (150/p₁) dp₁ = 150|ln p₁|₄² = 150 ln(0.5) ≈ **103.98**.
• **EV > CS > CV ✓ ⇒ both goods are normal.** EV-CV = 36.34 > 0 confirms substantial income effect.
**→ See:** answers-welfare.md for full derivation

### Question 4
**Q:** EV > CS > CV diagram (price decrease) and EV < CS < CV (price increase) — graphical analysis.
**A:**
• **Price decrease p₁↓ (normal good):** Three demand curves intersect at p₁⁰. At p₁¹ < p₁⁰: h(p,u¹) > x(p,w) > h(p,u⁰). EV = area left of h(p,u¹) → largest; CS = area left of x(p,w) → intermediate; CV = area left of h(p,u⁰) → smallest.
• **Price increase p₁↑ (normal good):** At p₁¹ > p₁⁰: h(p,u⁰) > x(p,w) > h(p,u¹). CV → largest; CS → intermediate; EV → smallest.
...
**A:**
• **Normal good ⇒ ∂x₁/∂m > 0** ⇒ positive income effect. Marshallian demand is flatter (includes income+substitution effects) than Hicksian at u⁰.
...
• **Point of bliss** = tangency between GUPF and highest attainable social indifference contour: slope(GUPF) = (∂W/∂U_A)/(∂W/∂U_B).
• **Does it ensure max SW?** YES — it identifies the constrained maximum given resources and SWF. **NO —** it depends on the specific SWF chosen. Different value judgements → different bliss points. Pareto optimality is necessary but not sufficient.
**→ See:** answers-welfare.md for full derivation
...
• **(iii) Cardinalist:** Diminishing MU of income ⇒ redistribute to equalize incomes ⇒ max SW. **Problem:** assumes identical utility functions — empirically false; ignores incentive effects.
**→ See:** answers-welfare.md for full derivation
...
• **GUPF derivation:** (1) PPC → product-mix. (2) For each product-mix, UPF = efficient utility pairs. (3) GUPF = envelope of all UPFs.
• **Point of bliss:** tangency of GUPF with highest SWF contour. Condition: slope(GUPF) = (∂W/∂U_A)/(∂W/∂U_B) AND MRPT = MRS^A = MRS^B (Pareto efficiency).
• Bliss point depends on SWF — different value judgements → different points.
**→ See:** answers-welfare.md for full derivation
...
• **(2) Dual conditions:** (i) National income growth → SW↑ (if distribution constant). (ii) Redistribution from rich to poor → SW↑ (diminishing MU, equal capacity). Criticism: equal capacity assumption is a value judgement, not science.
• **(3) Private vs Social net product (externalities):** Divergence → tax/subsidize. Criticism: hard to quantify; structural failures pervasive.
...
• **Uses:** (1) Compare states: W' > W ⇒ S' better than S. (2) Constrained optimization: max W subject to GUPF → bliss point. (3) Evaluate Pareto-incomparable changes (winners vs losers).
...
• **Can compensation be correct?**
  ...
  • **NO if MU differs:** Rich gaining £2000 (low MU) vs poor losing £1000 (high MU) → total utility may fall despite Kaldor-Hicks passing.
  • **Potential ≠ actual:** If compensation not paid, losers actually worse off.
...
**A:**
• **Setup:** Max U_A(X_A,Y_A) s.t. U_B(X̄-X_A, Ȳ-Y_A) ≥ Ū_B.
• **Lagrangian:** ℒ = U_A(X_A,Y_A) + λ[U_B(X̄-X_A, Ȳ-Y_A) - Ū_B].
• **FOCs:** ∂U_A/∂X_A = λ·∂U_B/∂X_B, ∂U_A/∂Y_A = λ·∂U_B/∂Y_B.
• **Dividing:** (∂U_A/∂X_A)/(∂U_A/∂Y_A) = (∂U_B/∂X_B)/(∂U_B/∂Y_B) ⇒ **MRS^A_xy = MRS^B_xy**.
...
• **Social optimum (joint profit max):** SMC₁=0.25q₁+5=15 ⇒ q₁=40; SMC₂=0.2q₂+7=15 ⇒ q₂=40. Total profit = 360.
• **Tax on Firm 1:** t = ∂C₂/∂q₁ = 0.05(40) = 2/unit. **Subsidy to Firm 2:** s = -∂C₁/∂q₂ = 0.2(40) = 8/unit.
...
• **Comparison:**
  • First: Competitive equilibrium → Pareto optimality. Policy: markets work.
  • Second: Pareto optimality → Competitive equilibrium with transfers. Policy: redistribute endowments, then let markets work.
...
• Other forms: Nash (W = ΠUᵢ), Bergson-Samuelson (general), weighted utilitarian (W = ΣαᵢUᵢ).
**→ See:** answers-welfare.md for full derivation
...
• **Limitations:** Potential ≠ actual compensation; ignores income distribution; assumes constant MU of money.
**→ See:** answers-welfare.md for full derivation
...
• **NO — does NOT guarantee SW maximization because:**
  • (1) Pareto optimality ≠ SW max — competition places us somewhere on GUPF, not necessarily the bliss point.
  • (2) Outcome depends on initial endowments — different endowments → different Pareto-optimal points.
...
| 4 | Graphically explain: external shock in product market → factor market → equilibrium restored in both markets. | 7/— | Batch 47 Q7b, Professor Q12 | 2022 |
...
• **Example:** Drought destroys wheat. PE: wheat price ↑. GE: wheat ↑ → bread ↑ → rice substitute ↑ → land shifts from cotton to wheat → cotton ↑ → textiles ↑ → factor markets adjust → all prices change.
...
• **(ii) Uniqueness:** Is there exactly one equilibrium? If E(P) is monotonic (dE/dP < 0 ∀ P), unique. Multiple intersections of D and S → multiple equilibria.
...
### Question 4
**Q:** Graphically explain: external shock in product market → factor market → equilibrium restored.
**A:**
• **Chain:** Taste shift toward X → D_X ↑ → P_X ↑ → excess profits in X → X expands → D for L and K ↑ → w_X ↑, r_X ↑. Simultaneously Y contracts → w_Y ↓, r_Y ↓.
...
• **Long run:** Entry into X shifts S_X right → P_X↓; exit from Y shifts S_Y left → P_Y↑. New GE at (P_X₂,P_Y₂,w₂,r₂) with normal profits restored.
...
| 2 | Graphically explain: external shock in product market affects factor market → equilibrium restored in both markets. | 10 | MainQ GE Q2b | — |
...
### Question 2
**Q:** Graphically explain: external shock → factor market → equilibrium restored in both markets.
**A:**
• Identical mechanism to Topic 6A Q4. Taste shift toward X: D_X↑ → P_X↑ → profits in X → expand → factor demand↑ → w↑, r↑. Y contracts → factor demand↓.
...
**A:**
• **(a) Production equilibrium:** MRTSˣ_LK = MRTSʸ_LK = w/r. Edgeworth production contract curve → PPC.
...
• **Example (coffee/tea):** Frost destroys coffee. PE: coffee price ↑. GE: coffee ↑ → switch to tea → tea ↑ → tea farmers expand → factor demand in tea ↑ → wages ↑ → income ↑ → further demand shifts → all markets adjust.
...
• **How GE reached:** (1) Production: MRTS = w/r → Edgeworth contract curve → PPC. (2) Output mix: MRPT = P_X/P_Y. (3) Consumption: MRS = P_X/P_Y → Edgeworth consumption contract curve. (4) Simultaneous: MRPT = MRS → production consistent with consumption. (5) All prices determined as ratios (numeraire needed). 18 equations with 18 unknowns, but one redundant (Walras' law) → numeraire resolves.
**→ See:** answers-ge-game-extern-trade.md for full derivation
...
• **Pareto efficiency condition (interior):** MRS_A = MRS_B ⇒ Y_A = X_A.
• **Contract curve:** Y_A = X_A (for interior allocations where B consumes both goods).
• **Endowment (10,50):** MRS_A = 50/10 = 5 ≠ 1 = MRS_B. Since MRS_A > MRS_B, A values Y more than B does → mutually beneficial trade possible. **Not Pareto-efficient.**
• **Allocation (80,50):** infeasible — total X = 60, so X_A cannot exceed 60. Outside the Edgeworth box.
• With B's linear utility, corner solutions possible: if P_X ≠ P_Y, B consumes only the cheaper good.
**→ See:** answers-ge-game-extern-trade.md for full derivation
...
• Walras' Law: one equation redundant → 17 independent eq, 18 unknowns. Numeraire (set one price = 1) → 17 unknowns. System determinate **in relative prices**.
...
• **Bertrand with differentiated products:** π₁ = (p₁-10)(100-2p₁+p₂). FOC: ∂π₁/∂p₁ = 120 - 4p₁ + p₂ = 0 ⇒ p₁ = 30 + p₂/4 (BR₁). Similarly p₂ = 30 + p₁/4 (BR₂).
• **Nash eq:** Solve simultaneously ⇒ p₁* = p₂* = 40. q₁ = q₂ = 60. π₁ = π₂ = 1800.
...
• **Backward induction:** At I's node, A gives 40 > -10 from F → I accommodates. At E's initial node, IN gives 40 > 0 from OUT → E enters.
...
• **Sustaining condition:** Cooperation PV = 3/(1-δ) ≥ deviation PV = 5 + 2δ/(1-δ) ⇒ δ ≥ 2/3.
• If players are sufficiently patient (δ ≥ ⅔), (Up,Left) is SPNE with grim trigger punishment to (Down,Right).
...
• **Overcoming the paradox:**
  • (1) **Incomplete information:** Small probability of "altruistic" type → Pass to build reputation (Kreps-Milgrom-Roberts-Wilson).
  ...
  • (4) **Social preferences:** Fairness/reciprocity → players Pass more than predicted.
...
**A:**
• **Tragedy of Commons (Hardin, 1968):** Common-pool resource without property rights → each individual rationally depletes the resource, destroying it for everyone.
• **Mechanism:** Each user captures full benefit of extra use but shares the cost — private MB > social MB ⇒ overuse.
• **Example:** Open-access fishery. Each fisher catches maximally ("if I don't, someone else will") → overfishing → stock collapse.
...
**A:**
• Steel mill: C_S(S,x), ∂C_S/∂x < 0 (pollution reduces cost). Fishery: C_F(F,x), ∂C_F/∂x > 0 (pollution raises cost).
• **(i) Individual max:** Steel sets ∂C_S/∂x = 0 (too much pollution, x*). Fishery takes x as given. Result: x* > x° (social optimum).
• **(ii) Joint max:** Merger internalizes externality. FOC: -∂C_S/∂x = ∂C_F/∂x (MB = MC). Less pollution: x° < x*. Total profit higher.
...
• **Asymmetric info:** Buyer's WTP = expected value = 0.5×10K + 0.5×5K = $7.5K. Peach owner reservation = $8K > $7.5K → peaches withdraw.
...
• **Separating PBE:** H acquires education (e=1), L does not (e=0). Firm beliefs: μ(H|e=1)=1, μ(H|e=0)=0. Condition: w_H - w_L ∈ [c_H, c_L].
...
• (2) **Missing market:** No price for A's influence on B's utility ⇒ First Welfare Theorem fails.
...
• **Free trade at P_W=600:** Q_d=190,000, Q_s=120,000 ⇒ **imports = 70,000**.
