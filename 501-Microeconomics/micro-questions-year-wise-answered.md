...
**A:** • Preferences are primitive, choices derived from them • Rationality requires **completeness** (∀ x,y: either x>=y or y>=x) and **transitivity** (if x>=y and y>=z then x>=z) • Reflexivity implied • Violations: Condorcet paradox, framing effects, just-noticeable differences
**→ See:** answers-consumer-theory.md §1A Q1

**Q1b:** Indifference set IND(x) = { y ∈ X : y ~ x }. Strong monotonicity ⇒ downward-sloping ICs.
**A:** • Strong mono: more of ANY good strictly ↑ utility • Proof by contradiction: upward-sloping IC ⇒ y >> x on same IC ⇒ y > x (strong mono) but y ~ x (IC definition) — contradiction • Rules out: upward-sloping ICs, thick ICs, bads
...
**A:** • Def 2 makes first equivalence definitional • Quasiconcave: u(λx+(1-λ)y) ≥ min{u(x), u(y)} • Proof: (⇒) if x,y ∈ UCS(z) then u(λx+(1-λ)y) ≥ min{u(x),u(y)} ≥ u(z) • (⇐) reverse the argument
**→ See:** answers-consumer-theory.md §1A Q4

**Q2c:** Debreu's theorem: monotonicity + continuity ⇒ ∃ continuous u(.) representing >=.
**A:** • Project each bundle x onto 45° line via unique IC intersection • Let m = max{xᵢ}, M = (m,...,m) • Define A = {t∈[0,m]: (t,...,t) <= x}, B = {t: (t,...,t) >= x} • Continuity ⇒ A,B closed, intersection non-empty • u(x) = t(x) where (t,...,t) ~ x
**→ See:** answers-consumer-theory.md §1A Q5

**Q3i:** u = x₁^α x₂^β. Lagrangian → FOCs → x₁ = [α/(α+β)]·(w/p₁), x₂ = [β/(α+β)]·(w/p₂).
**A:** • HD(0) in (p,w) ✓ • Walras' Law ✓ • ∂x₁/∂p₁ < 0 ✓ • ∂x₁/∂p₂ = 0 (zero cross-price) • ∂x₁/∂w > 0 (normal) • **Constant expenditure shares**: s₁ = α/(α+β), s₂ = β/(α+β)
**→ See:** answers-consumer-theory.md §1B Q1

**Q3ii:** v(p,w) = B·w^{α+β}·p₁^{-α}·p₂^{-β}, B = α^αβ^β/(α+β)^{α+β}.
**A:** • HD(0) in (p,w) • ↑ w, ↓ p_k • Quasiconvex in (p,w) • Roy's identity: x_k = -(∂v/∂p_k)/(∂v/∂w) • Verify: ∂v/∂p₁ = -αv/p₁, ∂v/∂w = (α+β)v/w ⇒ -∂v/∂p₁/∂v/∂w = αw/((α+β)p₁) = x₁ ✓
**→ See:** answers-consumer-theory.md §1B Q2

**Q3iii:** EMP: min p·x s.t. u(x) ≥ ū. FOCs → same tangency ratio as UMP.
**A:** • Hicksian demand: h₁ = ū^{1/(α+β)}·(α/β)^{β/(α+β)}·(p₂/p₁)^{β/(α+β)} • e(p,ū) = ū^{1/(α+β)}·(α+β)·(p₁/α)^{α/(α+β)}(p₂/β)^{β/(α+β)} • HD(0) in p • ∂h₁/∂p₁ < 0 • Duality: e(p,v(p,w)) = w, v(p,e(p,ū)) = ū
**→ See:** answers-consumer-theory.md §1B Q3

**Q4a:** u: X → R where x>=y ⇔ u(x) ≥ u(y). Only ranking matters (ordinality).
**A:** • Monotonicity: all goods ↑ ⇒ strictly better • Strong monotonicity: one good ↑ ⇒ strictly better • Any strictly ↑ transform f(u) represents same preferences • Examples: u = x^αy^β vs v = α ln x + β ln y
**→ See:** answers-consumer-theory.md §1A Q6

**Q4b:** Chain: Strong Mono ⇒ Mono ⇒ LNS.
**A:** • Strong Mono ⇒ Mono: if x_k ≥ y_k ∀k, either x_l > y_l for some l ⇒ x > y, or x = y ⇒ x ~ y. Either way x >= y ✓ • Mono ⇒ LNS: ∀ x, ε>0, construct y = (x₁+δ,...) with δ = ε/√N. Then y_k > x_k ∀k ⇒ y > x, and ||y-x|| = ε ✓
...
**A:** • HD ⇒ MRS constant on rays • Homothetic ⇒ MRS depends only on ratio x₁/x₂ • Engel curves are straight lines through origin • All homogeneous are homothetic; converse false (e.g., u = xy + a) • CD: HD(α+β) and homothetic
...
**A:** • EV = e(p⁰, u¹) − e(p⁰, u⁰) = e(p⁰, u¹) − w — uses old prices, new utility • CS = ∫ x₁(p₁, p̄₋₁, w) dp₁ (area under Marshallian demand) • CV & EV exact (Hicksian); CS approximate (Marshallian mixes income+substitution effects) • For zero income effect, all three equal
...
**A:** • Transformation function F(y) — implicit, F(y)=0 at boundary • Transformation frontier {y: F(y)=0} — technically efficient points • MRPT_kₗ = (∂F/∂y_k)/(∂F/∂y_l) = −dyₗ/dy_k
...
**A:** If Y closed & free disposal: • HD(1) in p: π(λp) = λπ(p) • Convex in p: π(αp+(1-α)p') ≤ απ(p)+(1-α)π(p') • Hotelling's Lemma: ∂π/∂p_k = y_k(p) • Non-↓ in output prices, non-↑ in input prices • Continuous in p • Law of Supply: ∂y_k/∂p_k ≥ 0
**→ See:** answers-demand-production.md §Topic 3B Q1

**Q3b:** α=½. p⁰=(4,4), m=300; p¹=(2,4), m=300. v⁰ = 37.5, v¹ ≈ 53.03.
**A:** • e(p,u) = 2u√(p₁p₂) • CV = w − e(p¹, u⁰) = 300 − 212.1 = **87.9** • EV = e(p⁰, u¹) − w = 424.24 − 300 = **124.24** • CS = ∫(150/p₁)dp₁ = 150 ln(0.5) = **103.98** • **EV > CS > CV** confirms both goods are **normal**
...
**A:** Sufficient condition violated when: • Non-monotone utility → corner solution (MRS ≠ price ratio) • Non-quasiconcave utility → tangency yields minimum, not max • ∇u = 0 (bliss point) → Walras' Law fails
...
**A:** • Homothetic ⇒ Engel curves linear through origin, expenditure shares constant, income elasticity = 1 for all goods • CD, linear, Leontief, CES are homothetic • Quasilinear (u = v(x)+y) is NOT homothetic
...
**A:** • **Existence**: E(P)=0 at some P>0 — requires continuity + boundary conditions (E(P)>0 at low P, <0 at high P) • **Uniqueness**: monotonic E(P) (dE/dP<0 everywhere); non-monotonic ⇒ multiple equilibria • **Stability**: dE/dP<0 ⇒ stable; dE/dP>0 ⇒ unstable • Draw D-S diagrams for all four cases
...
**A:** • Correct only if MU of money identical across individuals • If unequal (MU declines with income), Kaldor-Hicks can be wrong: rich gain £2000 (low MU), poor lose £1000 (high MU) → KH says pass, actual welfare falls • Potential ≠ actual compensation • Scitovsky's double test required
...
**A:** • FOCs: ∂U_A/∂X_A = λ·∂U_B/∂X_B, ∂U_A/∂Y_A = λ·∂U_B/∂Y_B • Dividing: MRS^A_xy = MRS^B_xy • Contract curve in Edgeworth box: all tangency points of A and B's ICs
**→ See:** answers-welfare.md §Topic 5B Q3

**Q2b:** C₁ = 0.1q₁²+5q₁−0.1q₂² (external economy from 2→1), C₂ = 0.2q₂²+7q₂+0.025q₁² (diseconomy from 1→2).
**A:** • Private: q₁=50, q₂=20, total π=307.5 • Social (joint max): q₁*=40, q₂*=40, total π=360 • Tax firm 1: t = ∂C₂/∂q₁ = 0.05(40) = **2/unit** • Subsidise firm 2: s = −∂C₁/∂q₂ = 0.2(40) = **8/unit** • **Social dividend = 360−307.5 = 52.5**
...
**A:** • **Utilitarian**: W = ΣUᵢ — linear social ICs (slope −1), sum-ranking, equity-blind • **Rawlsian**: W = min{Uᵢ} — L-shaped ICs, lexicographic priority to worst-off, extreme inequality aversion • Nash SWF: W = ΠUᵢ (rectangular hyperbola)
...
**A:** • q₁=100−2p₁+p₂, q₂=100−2p₂+p₁, MC=10 • BR₁: p₁=30+p₂/4, BR₂: p₂=30+p₁/4 • Solve: p₁*=p₂*=**40**, q₁=q₂=60, π₁=π₂=1800 • Bertrand with differentiated products → prices are strategic complements (BR slopes positive)
...
**A:** • Stage-game NE: (Down, Right) = (5,5) • Infinitely repeated with grim trigger: play (Up, Left) = (4,4) cooperatively; deviation → (Down, Right) forever • Sustaining condition: 4/(1−δ) ≥ 6 + 5δ/(1−δ) ⇒ δ ≥ 2/3 • For δ ≥ ⅔, cooperation on (4,4) is SPNE
...
**A:** • Private max: S sets −∂C_S/∂x = 0 → too much pollution • Joint max: −∂C_S/∂x = ∂C_F/∂x → efficient x° < x* • Coase Theorem: with property rights & zero transaction costs, bargaining reaches x° regardless of rights allocation • DWL = triangle between x° and x*
...
**A:** • Separating PBE: H acquires education (e=1), L does not (e=0). Firms: w(e=1)=high, w(e=0)=low. Requires w_H−w_L ∈ [c_H, c_L] • Pooling PBE: neither acquires education; off-equilibrium beliefs sustain it • Education is wastefully costly in separating eq. but restores information • Does NOT raise productivity — pure signal
...
**A:** • Completeness: ∀ x,y: can compare • Transitivity: no cycles • Rational preference → utility representation
**→ See:** answers-consumer-theory.md §1A Q1

**Q1b:** (Identical to MainQ 1b.) Indifference set IND(x) = {y: y~x}. Strong mono ⇒ ICs downward-sloping by contradiction.
**A:** • Upward-sloping IC ⇒ y >> x but y~x → contradiction with strong mono
**→ See:** answers-consumer-theory.md §1A Q2
...
**A:** • UCS convex ⇔ u quasiconcave: quasiconcavity definition directly mirrors convex UCS property
**→ See:** answers-consumer-theory.md §1A Q4

**Q2c:** (Identical to MainQ 2c.) Debreu: project bundle onto 45° line via IC intersection.
**A:** • u(x) = t where (t,...,t) ~ x • Continuity ensures unique t exists
**→ See:** answers-consumer-theory.md §1A Q5
...
**A:** • HD(0) in (p,w) • ↑ w, ↓ p_k • Roy's verified: ∂v/∂p₁ = −αv/p₁, ∂v/∂w = (α+β)v/w ⇒ x₁ = αw/((α+β)p₁) ✓
**→ See:** answers-consumer-theory.md §1B Q2

**Q3iii:** (Identical to MainQ 3iii.) h₁ = ū^{1/(α+β)}·(α/β)^{β/(α+β)}·(p₂/p₁)^{β/(α+β)}. e(p,ū) duality checks.
**A:** • EMP setup → same tangency ratio as UMP • Hicksian HD(0) in p • Compensated law of demand: ∂h/∂p < 0
**→ See:** answers-consumer-theory.md §1B Q3

**Q4a:** (Identical to MainQ 4a.) u: X→R representing >=. Monotonicity vs strong monotonicity distinction.
**A:** • Mono: all goods ↑ ⇒ better; Strong mono: one good ↑ ⇒ better • Only ranking matters (ordinal)
**→ See:** answers-consumer-theory.md §1A Q6

**Q4b:** (Identical to MainQ 4b.) Strong Mono ⇒ Mono ⇒ LNS.
**A:** • Strong Mono ⇒ Mono: one-good increase gives strict or weak preference • Mono ⇒ LNS: construct y = x+δ in ε-ball, all components larger ⇒ strictly better
**→ See:** answers-consumer-theory.md §1A Q7

**Q4c:** (Identical to MainQ 4c.) Homogeneous: u(ax)=a^k u(x). Homothetic: monotonic transform of homogeneous.
**A:** • HD ⇒ MRS constant on rays • Homothetic ⇒ Engel curves linear through origin • Homogeneous ⊂ Homothetic (converse false)
**→ See:** answers-consumer-theory.md §1A Q8
...
**A:** • Linear q = aK+bL: MRTS = b/a constant → σ = ∞ • Leontief: K/L fixed at kink → σ = 0 • CD: σ = 1
...
**A:** • Convex in p (not concave!) • Hotelling: ∂π/∂p_k = y_k(p) • Law of supply: ∂y_k/∂p_k ≥ 0
**→ See:** answers-demand-production.md §Topic 3B Q1
...
**A:** • e(p,u)=2u√(p₁p₂) • CV=87.9, EV=124.24, CS=103.98 • EV>CS>CV ⇒ both **normal goods** • Positive income effect (EV−CV=36.34)
**→ See:** answers-welfare.md §Topic 4 Q3

**Q7a:** (Same as MainQ GE Q1b.) Existence: E(P)=0 at P>0. Uniqueness: monotonic E(P). Stability: dE/dP<0.
**A:** • Existence: continuity + IVT • Uniqueness: dE/dP<0 ∀P • Stability: Walrasian tâtonnement • Arrow-Debreu conditions
...
**A:** • MRS_A = Y_A/X_A, MRS_B = 1 • Pareto efficiency: Y_A = X_A (interior) • Endowment MRS_A=5 ≠ 1 ⇒ **not efficient**, scope for trade • Contract curve: Y_A = X_A (0 ≤ X_A ≤ 60) • (X_A=80, Y_A=50) infeasible (total X=60)
...
**A:** • MRS = u_x/u_y • Along IC: dMRS/dx = [u_x²u_yy − 2u_xu_yu_xy + u_y²u_xx] / u_y³ • Numerator = −(bordered Hessian condition) • dMRS/dx < 0 ⇔ numerator < 0 ⇔ quasiconcavity holds
**→ See:** answers-consumer-theory.md §1A Q9

**Q1b:** u(G,V,C) = min(G,V,C). Negroni 1:1:1.
**A:** • Marshallian: G* = w/(p_G+p_V+p_C) (all three identical) • Hicksian: h_C(p,u) = ū (independent of prices) • Expenditure: e(p,u) = u·(p_G+p_V+p_C) • HD(0) for Marshallian, HD(0) for Hicksian in p
**→ See:** answers-consumer-theory.md §1B Q6

**Q1c:** u = x + ln(y). MRS = y. Tangency: y* = p_x/p_y. x* = I/p_x − 1.
**A:** • y is income-inelastic (∂y/∂I = 0) • x absorbs all extra income • If I < p_x: corner solution, only y consumed • Quasilinear: zero income effect for y
**→ See:** answers-consumer-theory.md §1B Q7

**Q2a:** u = x + log(y). MRS = y. dMRS/dx = dy/dx = −y < 0. Yes, convex ICs.
**A:** • Bordered Hessian: 2u_xu_yu_xy − u_x²u_yy − u_y²u_xx = 1/y² > 0 ✓ • Quasiconcave confirmed
**→ See:** answers-consumer-theory.md §1A Q10

**Q2b:** X = αP_X^γ P_Z^δ P_W^ε I^η. Cross-price with Z: ε = δ. Income elasticity = η.
**A:** • δ=0.8>0 ⇒ Z is **gross substitute** • η not given directly — check coefficients • Own-price |γ|=0.4<1 ⇒ inelastic • P_X↓ ⇒ expenditure ↓ (inelastic good) • If P_W↓5% and ε>0 ⇒ Q↓; if ε<0 ⇒ Q↑
**→ See:** answers-demand-production.md §Topic 2 Q1

**Q2c:** u = x − 3y. MU_y = −3 < 0 — y is a **bad**. Corner solution: y=0, x=400/20=20.
**A:** • Do NOT use Lagrangian (identifies minimum) • MU_y<0 ⇒ consumer chooses zero y • u(20,0)=20
**→ See:** answers-consumer-theory.md §1B Q8

**Q3a:** Q = 10 + 1.3P_B + 0.4P_M − 3P_F + 2.2I. The coefficient on P_B is **positive** (+1.3).
**A:** • ∂Q/∂P_B = 1.3 > 0 ⇒ upward-sloping demand • Violates Law of Demand • Likely misspecified model
**→ See:** answers-demand-production.md §Topic 2 Q2

**Q3b:** Q = 150P_B^{−1.5} P_{r1}^{0.75} P_{r2}^{−0.25} I^{0.8} A^{0.05}.
**A:** • r2 (ε=−0.25) is French fries (complement) • r1 (ε=0.75) is substitute (Big Mac) • η=0.8 ⇒ necessity • Advertising: 0.05×50%=**2.5%** ↑ • FF price ↓50% ⇒ Whopper Q ↑ by 12.5% • Profit-max P = MC/(1+1/ε) = 1/(1−⅔) = **$3.00** • BK must lower price by **5%** to offset McDonald's 10% cut
...
**A:** • (1) c(λw,q)=λc(w,q) • (2) q₁>q₀ ⇒ c(q₁)≥c(q₀) • (3) Firm substitutes away from expensive inputs → cost rises less than linearly • (4) ∂c/∂w_k = z_k(w,q) (conditional factor demand)
**→ See:** answers-demand-production.md §Topic 3A Q5

**Q4b:** Sausage: 3 oz pork + 2 oz veal → 5 oz sausage. Q = min{P/3, V/2}.
**A:** • Production: Q = min{P/3, V/2} • Conditional demand: P(Q)=3Q, V(Q)=2Q • Cost: c(Q) = 3p_P·Q + 2p_V·Q = 5Q (if p_P=p_V=1) • Demand Q=200P^{−6}: MR=MC ⇒ P*=**$6**
**→ See:** answers-demand-production.md §Topic 3A Q6

**Q5a:** Q = (KL)^{1/3}, K̄=64. SR: Q=4L^{1/3}.
**A:** • L = Q³/64 • SRTC = Q³/64 + 64 (w=r=1) • Shutdown: min AVC = 0 (any P>0 produces) • SR supply: Q = √(64P/3) • DRS (⅓+⅓=⅔<1) • σ=1 (CD) • LR: L=(r/w)^{½}Q^{3/2}, K=(w/r)^{½}Q^{3/2} • LRTC=2√(wr)·Q^{3/2} • LR supply: Q = P²/(9wr)
...
**A:** • SR supply per firm: q=P−2 (P≥2) • Industry: Qs=1000(P−2) • Eq: P=23, Q=21,000, each q=21, π=140.5 • $20 tax: P_b=33, P_s=13, Q=11,000 • DWL = ½×20×10,000 = **100,000** • Tax split evenly (equal slopes)
**→ See:** answers-demand-production.md §Topic 3C Q1

**Q6b:** Three-good world: Z₂ = −3 + p₃/p₂ + p₁/p₂, Z₃ = 3p₁/p₃ + 4p₂/p₃ − 7.
**A:** • Walras' law: p₁Z₁+p₂Z₂+p₃Z₃=0 ⇒ Z₁ = −4 + (6p₃−p₂)/p₁ • Solve Z₂=0, Z₃=0: p₂/p₁=10/17, p₃/p₁=13/17 • Z₁=0 automatically by Walras' law
...
**A:** • First Welfare Theorem fails when externalities present • Market failure: missing market for the externality • Standard MRS ≠ MRS condition no longer sufficient
...
**A:** • Direct proof: total differentiate F=0 → MRPT = (∂F/∂q_x)/(∂F/∂q_y) • Profit max: p_x=MC_x, p_y=MC_y • MRPT = p_x/p_y = MC_x/MC_y ✓
...
**Consumer Theory:**
**A:** • Preference approach: completeness + transitivity → rational preference • Convexity: chord above IC; strict convexity: chord strictly above • LNS: ∀ x, ε>0, ∃ y within ε with y > x • Mono: all goods ↑ ⇒ strictly better; Strong mono: one good ↑ ⇒ strictly better • Utility function: x>=y ⇔ u(x)≥u(y); ordinal, unique up to monotonic transform
**→ See:** answers-consumer-theory.md §1A

**Utility Functions:**
**A:** • CD: u=x^αy^β — constant shares, σ=1 • Perfect substitutes: u=ax₁+bx₂ — linear ICs, σ=∞, corner solutions • Perfect complements: u=min{ax₁,bx₂} — right-angle ICs, σ=0, fixed proportions • Quasilinear: u=v(x)+y — zero income effect for x
**→ See:** answers-consumer-theory.md §1A and §1B

**Walrasian Demand:**
**A:** • Properties: HD(0), Walras' Law (p·x=w by LNS), convex set (convex pref), single-valued (strictly convex) • Sufficient conditions for interior solution: (1) u quasiconcave (2) u monotone (3) ∇u ≠ 0 • Violations → corner solutions or minima
**→ See:** answers-consumer-theory.md §1B Q4
...
**Production:**
**A:** • Production set: closed, no free lunch, free disposal, possibility of inaction, convexity • Profit function (Y closed & free disposal): HD(1) in p, convex, Hotelling's Lemma, law of supply • Properties from Y → properties of π(p)
**→ See:** answers-demand-production.md §Topic 3A Q2&3, §Topic 3B Q1

**Cobb-Douglas Applications:**
**A:** • Walrasian demand: x₁=αw/(α+β)p₁, x₂=βw/(α+β)p₂ • Indirect utility: v=B·w^{α+β}·p₁^{-α}·p₂^{-β} • Roy's identity: x_k = −(∂v/∂p_k)/(∂v/∂w) • Expenditure function: e = ū^{1/(α+β)}·(α+β)(p₁/α)^{α/(α+β)}(p₂/β)^{β/(α+β)} • Hicksian demand derived from EMP
