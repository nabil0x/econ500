# Microeconomics — Resource Files

| # | File | Type | Content |
|---|------|------|---------|
| 1 | `A_Koutsoyiannis_Modern_Microeconomics_Se (1).pdf` | Textbook | Modern Microeconomics — Koutsoyiannis (full text) |
| 2 | `Felix_Munoz-Garcia_-_Advanced_Microeconomic_Theory__An_Intuitive_Approach_with_Examples-The_MIT_Press_(2017)[1].pdf` | Textbook | Advanced Micro Theory — Munoz-Garcia (MIT Press) |
| 3 | `chapter-1-preference-and-choice.pdf` | Chapter | Preferences, utility, choice theory |
| 4 | `chapter-2-demand-theory.pdf` | Chapter | Demand theory |
| 5 | `chapter-3-demand-theory-applications.pdf` | Chapter | Demand applications |
| 6 | `chapter-4-production-theory.pdf` | Chapter | Production theory |
| 7 | `Introduction 501.docx` | Notes | Intro to economics — scarcity, markets, price mechanism |
| 8 | `Footnotes on different topics micro.docx` | Notes | Micro topic summaries |
| 9 | `Footnotes on micro different topics.docx.pdf` | Notes | Micro topic summaries (PDF) |
| 10 | `Micro questions 49 given by professor.docx.pdf` | Questions | 49 practice questions from professor |
| 11 | `Micro questions Given by professor.docx.pdf` | Questions | Additional professor questions |
| 12 | `Practice questions for mid 1 and 2.pdf` | Questions | Midterm practice (scanned) |
| 13 | `A Graphical Illustration of the Path to General Equilibrium.docx.pdf` | Paper | GE path illustration |
| 14 | `Static Properties of a General Equilibrium State.docx.pdf` | Paper | GE static properties |
| 15 | ~~`WelfareChange (CV, EV and CS).pdf`~~ | Paper | ⛔ File unavailable — superseded by [reference notes](../welfare-change-reference.md) |
| 16 | `Welfare Economics 1.docx` | Notes | Welfare economics notes |
| 17 | `Welfare Economics.pptx` | Slides | Welfare economics slides |
| 18 | `welfare sheet.pdf` | Sheet | Welfare reference sheet (scanned) |
| 19 | `welfare sheet Pigou.pdf` | Sheet | Pigouvian welfare sheet (scanned) |
| 20 | `master-notes-501/1-8.pdf` | Solved Tutorial | Q1-8 consumer theory (scanned) |
| 21 | `master-notes-501/9,10,11.pdf` | Solved Tutorial | Q9: concavity, Q10: utility functions, Q11: homogeneity |
| 22 | `master-notes-501/12-17.pdf` | Solved Tutorial | Q12-17 demand/production/welfare (scanned) |
| 23 | `master-notes-501/501 18-22.pdf` | Solved Tutorial | Q18-22 welfare/GE (scanned) |
| 24 | `master-notes-501/501_chapter 1 solve_1st tt.pdf` | Solved Tutorial | Ch.1 tutorial test (scanned) |
| 25 | `master-notes-501/Social welfare 1-17.pdf` | Solved Tutorial | 11 long-form welfare/GE answers with figures |
| 26 | `master-notes-501/Welfare Q no5 ans.pdf` | Solved Tutorial | Alternative Pigou answer |

> **Tip:** The reference notes at `../welfare-change-reference.md` cover CV/EV/CS in detail (the source PDF is unavailable). The `master-notes-501/` files are student's personal solved tutorial sheets — all extractable content is already covered in the existing answer files (see [master-notes-501/README.md](./master-notes-501/README.md) for full content match).

---

## Extracted Text

All PDFs, DOCX, and PPTX files have been extracted to plain text in the `./extracted_text/` folder for easy searching and reference when writing answers.

| Extracted File | Words | Source Type | Notes |
|----------------|-------|-------------|-------|
| `A_Koutsoyiannis_Modern_Microeconomics.txt` | 238,850 | Textbook | Full textbook, all chapters |
| `Felix_Munoz-Garcia_Advanced_Microeconomic_Theory.txt` | 286,705 | Textbook | Full textbook, all chapters |
| `chapter-1-preference-and-choice.txt` | 12,754 | Chapter | Preferences, utility, choice theory |
| `chapter-2-demand-theory.txt` | 11,741 | Chapter | Demand theory |
| `chapter-3-demand-theory-applications.txt` | 7,872 | Chapter | Demand applications |
| `chapter-4-production-theory.txt` | 12,954 | Chapter | Production theory |
| `welfare sheet.txt` | 13,864 | Scanned sheet | Welfare economics (OCR, may have errors) |
| `welfare sheet Pigou.txt` | 13,864 | Scanned sheet | Same content as welfare sheet |
| `Welfare Economics.txt` | 3,716 | Slides | Welfare economics PPTX content |
| `Welfare Economics 1.txt` | 1,516 | Notes | Welfare notes |
| `Footnotes on different topics micro.txt` | 1,813 | Notes | Micro topic summaries |
| `Footnotes on micro different topics.docx.txt` | 5,195 | Notes | Extended micro notes |
| `Introduction 501.txt` | 1,873 | Notes | Intro to economics + micro theory |
| `A Graphical Illustration of the Path to General Equilibrium.docx.txt` | 1,731 | Paper | GE path illustration |
| `Static Properties of a General Equilibrium State.docx.txt` | 2,345 | Paper | GE static properties |
| `Micro questions Given by professor.docx.txt` | 297 | Questions | 17 professor questions (clear text) |
| `Micro  questions 49 given by professor.docx.txt` | 203 | Questions | 10-question subset (clear text) |
| `Practice questions for mid 1 and 2.txt` | 570 | Scanned | Handwritten Bengali+English (limited OCR) |
| `Masters_Notes_501/9,10,11.txt` | 2,136 | Solved Tutorial | Q9-11: concavity, utility functions, homogeneity |
| `Masters_Notes_501/Social welfare 1-17.txt` | 11,000 | Solved Tutorial | 11 long-form welfare/GE answers |
| `Masters_Notes_501/Welfare Q no5 ans.txt` | 422 | Solved Tutorial | Alternative Pigou answer |

> **Usage:** When writing answers to micro questions, reference these extracted texts as the primary source material. The two textbooks (Koutsoyiannis, Munoz-Garcia) and four chapter PDFs provide the theoretical backbone.
