...
1. **Equation** — the formula you need to recognize.
...
For exam writing, do not only state the final formula. Start from the optimization problem, write the Lagrangian or definition, take the first-order condition, and then explain the economic meaning in one sentence.
...
**Logic:** Completeness is a comparability rule: for any two bundles, at least one weak preference statement must be true. Transitivity is a consistency rule: once preferences rank $x$ above $y$ and $y$ above $z$, they cannot rank $z$ above $x$ without contradiction.
...
**Logic:** SM compares bundles good-by-good and requires strict preference when one bundle has at least as much of everything and more of something. M only requires strict preference when one bundle has strictly more of every good. LNS is weaker: it does not say "more is always better"; it only says no chosen bundle is a local bliss point.
...
**Logic:** If preferences are homothetic, scaling a bundle does not change the shape of indifference curves along rays from the origin. Solving demand at wealth $1$ gives a reference bundle. At wealth $w$, the consumer chooses the same proportions scaled by $w$.
...
**Derivation / logic:**
1. Quasiconcavity means the upper contour set $\{x:u(x)\ge c\}$ is convex.
2. Therefore, if $x$ and $y$ are both at least as good as level $c$, any mixture $\lambda x+(1-\lambda)y$ is also at least as good as $c$.
3. In two-good problems, this appears as a diminishing MRS condition.
4. For differentiable utility, the bordered Hessian gives a practical derivative test for the same curvature property.
...
**Derivation:**
1. Start from the utility maximization problem:
   $$\max_x u(x)\quad\text{s.t. }p\cdot x\le w.$$
2. Under local nonsatiation, the consumer spends all wealth, so use $p\cdot x=w$.
3. Write the Lagrangian:
   $$\mathcal L=u(x)+\lambda(w-p\cdot x).$$
4. Differentiate with respect to good $k$:
   $$\frac{\partial \mathcal L}{\partial x_k}=u_{x_k}-\lambda p_k=0.$$
5. Rearrange:
   $$u_{x_k}=\lambda p_k\quad\Rightarrow\quad \frac{u_{x_k}}{p_k}=\lambda.$$
6. For two goods, divide the two FOCs:
...
**Derivation / logic:**
1. Walrasian demand is the bundle that solves the UMP:
   $$x(p,w)\in \arg\max_x\{u(x):p\cdot x\le w\}.$$
2. If prices and wealth are both multiplied by $\alpha>0$, the budget set is unchanged:
   $$p\cdot x\le w\iff \alpha p\cdot x\le \alpha w.$$
3. Therefore the chosen bundle is unchanged:
   $$x(\alpha p,\alpha w)=x(p,w).$$
4. Indirect utility is utility evaluated at the optimal bundle:
   $$v(p,w)=u(x(p,w)).$$
...
**Derivation:**
1. From the UMP Lagrangian, the multiplier $\lambda$ is the marginal utility of wealth:
   $$\frac{\partial v}{\partial w}=\lambda.$$
2. By the envelope theorem, the direct effect of increasing price $p_k$ is:
   $$\frac{\partial v}{\partial p_k}=-\lambda x_k.$$
3. Divide the price effect by the wealth effect:
   $$-\frac{\partial v/\partial p_k}{\partial v/\partial w}
   =-\frac{-\lambda x_k}{\lambda}=x_k.$$
...
**Derivation:**
1. Start from the expenditure minimization problem:
   $$\min_x p\cdot x\quad\text{s.t. }u(x)\ge \bar u.$$
2. Write the Lagrangian:
   $$\mathcal L=p\cdot x+\mu(\bar u-u(x)).$$
3. For an interior optimum:
   $$\frac{\partial \mathcal L}{\partial x_k}=p_k-\mu u_{x_k}=0.$$
4. Therefore:
   $$p_k=\mu u_{x_k}.$$
5. The cost-minimizing bundle is Hicksian demand $h(p,u)$, and the minimized cost is:
   $$e(p,u)=p\cdot h(p,u).$$
...
**Derivation / logic:**
1. The UMP asks: "With wealth $w$, what is the highest utility?" The answer is $v(p,w)$.
2. If the consumer needs to reach exactly that utility, the least expenditure required is the original wealth:
   $$e(p,v(p,w))=w.$$
3. The EMP asks: "What is the least wealth needed to reach utility $u$?" The answer is $e(p,u)$.
4. If that amount is given to the consumer, the highest attainable utility is exactly $u$:
   $$v(p,e(p,u))=u.$$
...
**Derivation:**
1. Link Hicksian and Walrasian demand using duality:
   $$h_k(p,u)=x_k(p,e(p,u)).$$
2. Differentiate both sides with respect to $p_j$:
   $$\frac{\partial h_k}{\partial p_j}
   =
   \frac{\partial x_k}{\partial p_j}
   +
   \frac{\partial x_k}{\partial w}\frac{\partial e}{\partial p_j}.$$
3. Apply Shephard's Lemma:
   $$\frac{\partial e}{\partial p_j}=h_j.$$
4. At $u=v(p,w)$, Hicksian and Walrasian demands coincide, so $h_j=x_j$.
5. Substitute and rearrange:
   $$\frac{\partial x_k}{\partial p_j}
   =
   \frac{\partial h_k}{\partial p_j}
   -
   x_j\frac{\partial x_k}{\partial w}.$$
...
**Logic:**
- HD(0): multiplying all relevant prices and income by the same factor leaves the real constraint unchanged.
- HD(1): multiplying all prices by the same factor multiplies total expenditure, cost, or profit value by that factor.
- Euler test: if $f$ is HD($r$), then $\sum_i p_i f_i(p)=r f(p)$.
...
1. Monopoly profit is:
   $$\pi(q)=P(q)q-C(q).$$
2. FOC:
   $$MR=MC.$$
3. Marginal revenue can be written as:
...
**Derivation of cost-minimizing MRTS:**
1. Minimize cost:
   $$\min_{L,K} wL+rK\quad\text{s.t. }f(L,K)=q.$$
2. Lagrangian:
   $$\mathcal L=wL+rK+\lambda(q-f(L,K)).$$
3. FOCs:
   $$w=\lambda MP_L,\qquad r=\lambda MP_K.$$
4. Divide:
...
**Derivation / logic:**
1. Cost minimization gives conditional factor demand:
   $$z(w,q)\in \arg\min_z\{w\cdot z:f(z)\ge q\}.$$
2. The minimized value is:
   $$c(w,q)=w\cdot z(w,q).$$
3. Envelope logic gives production Shephard's Lemma:
   $$\frac{\partial c(w,q)}{\partial w_k}=z_k(w,q).$$
4. Profit maximization gives net supply:
   $$y(p)\in \arg\max_y p\cdot y.$$
5. The maximized value is:
   $$\pi(p)=p\cdot y(p).$$
6. Envelope logic gives Hotelling's Lemma:
...
2. New utility:
   $$u^1=v(p^1,w).$$
3. CV uses new prices and asks how much money is needed at $p^1$ to keep the consumer at old utility $u^0$:
   $$CV=w-e(p^1,u^0).$$
4. EV uses old prices and asks how much money at $p^0$ would make the consumer as well off as after the change:
   $$EV=e(p^0,u^1)-w.$$
5. CS uses ordinary demand, not compensated demand:
   $$CS=\int_{p^0}^{p^1}x(p,w)\,dp.$$
...
**Logic:** For a normal good, a price fall raises real income and increases demand. The Hicksian demand curve at the new, higher utility lies farther out than Marshallian demand, while the Hicksian demand curve at the old utility lies closer in. That gives the price-decrease ordering $EV>CS>CV$. A price increase reverses the comparison.
...
**Derivation / logic:**
1. Exchange efficiency: if $MRS^A\ne MRS^B$, two consumers value goods differently at the margin, so mutually beneficial trade is still possible.
2. Production efficiency: if $MRTS^X\ne MRTS^Y$, inputs can be reallocated between firms to raise output.
3. Product mix efficiency: if $MRPT\ne MRS$, the economy is producing the wrong mix of goods relative to consumers' marginal willingness to trade.
...
**Logic:** The First Welfare Theorem moves from decentralized competitive behavior to efficiency. The Second Welfare Theorem moves in the opposite direction: from a desired Pareto efficient allocation to a supporting price system, provided convexity allows separation by prices.
...
**Logic:** Kaldor-Hicks asks whether gains are large enough that winners could compensate losers. Scitovsky adds the reverse question: could the losers compensate the winners to prevent the change? If both directions pass, the criterion is contradictory, so the reversal test is needed.
...
**Derivation / logic:** For each agent, the value of planned purchases equals the value of available wealth/endowment when the budget binds. Adding across all agents, total value demanded must equal total value supplied. Therefore the value of aggregate excess demand is zero:
$$\sum_i P_iZ_i(P)=0.$$
...
**Logic:** Equilibrium is a zero of excess demand. If excess demand is continuous and changes sign between very low and very high prices, the Intermediate Value Theorem guarantees a crossing. Stability asks whether price adjustment moves back toward that crossing after a disturbance.
...
**Logic:** A point is on the exchange contract curve when the two consumers' indifference curves are tangent. A point is on the production contract curve when the two firms' isoquants are tangent. Non-tangency means a mutually beneficial reallocation is still available.
...
**Logic:** Nash equilibrium is a fixed point in best responses: each player's chosen strategy must be optimal given the other players' chosen strategies. In sequential games, backward induction applies this same best-response idea from the final decision node back to the start.
...
**Derivation of repeated-game condition:**
1. Cooperation is sustainable when the value of cooperating is at least the value of deviating once and then being punished.
2. The one-period gain from deviation is $\pi_{dev}-\pi_{coop}$.
3. The future loss from punishment is weighted by patience, $\delta$.
4. Rearranging the no-deviation condition gives:
   $$\delta \ge \frac{\pi_{dev}-\pi_{coop}}{\pi_{dev}-\pi_{pun}}.$$
**Logic of Spence condition:** A separating equilibrium requires education to be worthwhile for high-productivity workers but not worthwhile for low-productivity workers. That is why the wage premium must lie between the two education costs.
...
**Logic:**
- Pigou: tax the external damage so private marginal cost equals social marginal cost.
- Coase: if bargaining is costless and rights are clear, parties internalize the externality themselves.
- Tariff: deadweight loss is a triangle, so area equals one-half times base times height:
